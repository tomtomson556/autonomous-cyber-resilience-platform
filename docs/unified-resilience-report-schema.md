...
produces `UNKNOWN`, not a confirmed failure. Version 1 may optionally evaluate
RTO from one exact, unambiguous, validated `restore-test-evidence/v1` match.
Backup existence, successful backup sessions, restore points, and RPO success
do not prove recoverability or RTO compliance.
